...
# user can curate studies that list the user as a study curator
curateable_studies[p] if {
	some p in all_studies
...
# if user is a site curator, they can access all studies that allow curate access for this method, path
else := all_studies if {
...
# if user is a study_curator, they can access studies that allow curate access for them for this method, path
else := curateable_studies if {
	input.body.method = "GET"
	regex.match(paths.curate.get[_], input.body.path) == true
}
